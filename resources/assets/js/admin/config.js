 ...
 */
$(document).on('submit', '#update-config', function(event){
    errors.clear();
    let data = Helper.getFormResults(this);
    Helper.startLoading();

    if(data['id'] != undefined) {
        axios.put(base_api +'/configs/'+ data['id'], data)
        ...
        axios.post(base_api +'/configs', data)
            .then(function (response) {
                var entity_id = response.data.data;
                window.location.href = base_api + '/configs/'+entity_id;
                Helper.endLoading();
            })
            .catch(function (error) {
                Helper.endLoading();
...
$("#btn-delete-entity").on('click', function(evt) {
    ...
    swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover this config!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
    })
        .then((willDelete) => {
            if (willDelete) {
                ...
                axios.delete(base_api + '/configs/'+entity_id, {})
                    .then(function (response) {
                        window.location.href = base_api + '/configs';
                    })
                    .catch(function (error) {
                        console.log(error);
                    });
...
$("#search-form").submit(function( event ) {
    ...
    window.history.pushState("ajax", "Search", base_api + '/configs?'+query);

    axios.get(base_api + '/configs/search?'+query)
        .then(function (response) {
