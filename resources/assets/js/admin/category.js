...
$(document).on('submit', '#update-category', function(event){
    ...
    if(data['id'] != undefined) {
        axios.put(base_api +'/categories/'+ data['id'], data)
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                axios.delete(base_api + '/categories/'+entity_id, {})
                    .then(function (response) {
                        window.location.href = base_api + '/categories';
                    })
                    .catch(function (error) {
                        console.log(error);
                    });
...
$("#search-form").submit(function( event ) {
    ...
    window.history.pushState("ajax", "Search", base_api + '/categories?'+query);

    axios.get(base_api + '/categories/search?'+query)
        .then(function (response) {
